...
{
    // Number of particles in the last frame of the dust animation (size of initialParticlePositions, minus particleCountSecondLastFrame)
    uint particleCountLastFrame : packoffset(c0);

    // Number of particles in the second to last frame of the dust animation
    uint particleCountSecondLastFrame : packoffset(c0.y);

    // Number of matching particle positions (size of matchPositions)
    uint matchPositionsCount : packoffset(c0.z);

    // Offset where the second to last frame's RNG starts, relative to the start of a dust animation
    uint lastFrameRngOffset : packoffset(c0.w);

    // How much RNG should be skipped between frames due to the battle text, starting from the second to last frame
    uint initialRngSkipAmount : packoffset(c1);
};
...
void main(uint3 GlobalInvocationID : SV_DispatchThreadID)
{
    ...
    // Simulate last frame's particles
    for (uint i = 0; i < particleCountLastFrame; i++)
    {
        ...
        // Use PRNG to simulate hspeed/gravity over time
    ...
    // Simulate second to last frame's particles
    rngPosition -= (particleCountSecondLastFrame * 2) + initialRngSkipAmount;
    uint totalParticles = particleCountLastFrame + particleCountSecondLastFrame;
    for (uint i = particleCountLastFrame; i < totalParticles; i++)
    {
        // Get initial particle position
        uint pos = initialParticlePositions[i];
        float px = (float)(pos >> 16);
        ...
        // Use PRNG to simulate hspeed/gravity over time
        uint relativeIndex = i - particleCountLastFrame;
        float gravity = rng_to_float(rngValues[rngPosition + (relativeIndex * 2)], 0.5) + 0.2;
        float hspeed = rng_to_float(rngValues[rngPosition + ((relativeIndex * 2) + 1)], 4.0) - 2.0;
        px += hspeed * 11.0;
        py -= gravity * 66.0;
